...
define("event", top.dispatchEvent ?  ["$node"] : ["$node","$event_fix"],function(){
    ...
    $.mix(facade,{
        ...
        curry: function( event ){// 用于对用户回调进行改造
            var fn =  function( e ){//真正的事件对象
                var ctarget = event.target//原来绑定事件的对象
                var queue = ( $._data( ctarget, "events") || [] ).concat();
                var args = [ event ].concat(  [] ), result;
        ...
        fix: function(event, real){//level2 API 用于修复事件对象的属性与方法,主要能IE678, FF用
            if( !event._fix ){
                event.originalEvent = real;
                event._fix = true;
                event.timeStamp = Date.now();
                for( var p in real ){
                    if( (p in event) ||  /^[A-Z_]+$/.test(p) || typeof real[p] == "function"){
                        continue;//去掉所有方法与常量
                    }
                    event[p] = real[p]
                }
    ...
    if( bindTop ){//事件系统三大核心方法之一，触发事件
        facade.fire = function( init ){
            var bindTarget = $["@bind"] in this, detail = {}
            ...
            detail._args = [].slice.call( arguments, 1 ) ;
