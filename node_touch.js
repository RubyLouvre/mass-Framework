...
// 节点操作模块
//==================================================
define("node", ["support", "class", "data", "query_neo"], function($) {
    var rtag = /^[a-zA-Z]+$/,
        //取得其tagName
    ...
    $.init = function(expr, context) {
        ...
        //分支3:  处理节点参数
        if (expr.nodeType) {
            dom.ownerDocument = expr.nodeType === 9 ? expr : expr.ownerDocument;
            dom.push(expr);
        } else if (typeof expr === "string") {
            doc = dom.ownerDocument = !context ? document : getDoc(context, context[0]);
            var scope = context || doc;
            expr = expr.trim();
            if (expr.charAt(0) === "<" && expr.charAt(expr.length - 1) === ">" && expr.length >= 3) {
                expr.test(rparse);
                nodes = $.parseHTML(expr, RegExp.$1); //分支5: 动态生成新节点
            ...
            dom.selector = expr;
            [].push.apply(dom, nodes);
        } else { //分支8：处理数组，节点集合或者mass对象或window对象
            dom.ownerDocument = getDoc(expr[0]);
            [].push.apply(dom, $.isArrayLike(expr) ? expr : [expr]);
        }
        return dom;
    }
    $.fn = $.prototype = [];
    $.extend = $.fn.extend = $.mix;
    $.fn.extend({
        mass: $.mass,
        size: function() {
            return this.length;
        ...
        get: function(num) { //取得与索引值相对应的节点，若为负数从后面取起，如果不传，则返回节点集的纯数组
            return !arguments.length ? [].slice.call(this) : this[num < 0 ? this.length + num : num];
        },
    ...
    function cloneNode(node, dataAndEvents, deepDataAndEvents) {
        if (node.nodeType === 1) {
            var neo = node.cloneNode(true), //复制元素的attributes
                src, neos, i;
            if (dataAndEvents) {
    ...
    $.fn.extend({
        ...
        closest: function(expr, context) {
            ...
            var nodes = $(expr, context || this.ownerDocument);
            //遍历原mass对象的节点
        ...
        index: function(expr) {
            ...
            return this.indexOf(expr.mass ? expr[0] : expr);
        }
    });
