...
// 节点操作模块
//==================================================
define("node", ["support", "class", "query", "data"], function($) {
    var rtag = /^[a-zA-Z]+$/,
        rtagName = /<([\w:]+)/,
        //取得其tagName
    ...
    if(typeof $.html.dataset === "undefined") {
        var elemCtrProto = window.HTMLElement["prototype"]
        Object.defineProperty(elemCtrProto, "dataset", {
            get: function() {
                var ret = {}
                for(var i = 0, attrs = this.attributes, attr; attr = attrs[i++];) {
                    var name = attr.name;
                    if(!name.indexOf("data-")) {
                        ret[$.String.camelize(name.slice(5))] = attr.value
                    }
                }
                return ret;
            },
