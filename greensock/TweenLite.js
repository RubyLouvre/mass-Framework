...
(function(window) {
    ...
    "use strict";
    //创建命名空间，这不符合JS界的惯例。现在JS都淡化长命名空间，只暴露一个层次很小的对象或函数作为空名空间
    //因此很少有这样的方法
 ...
 */
    ...
    _class("Ticker", function(fps, useRAF) {
        ...
        this.fps = function(value) {
            ...
            _fps = value;
            _gap = 1 / (_fps || 60);//求出每帧要用多少秒
            _nextTime = this.time + _gap;//求出下一次在什么时候触发
            _req = (_fps === 0) ? function(f){} : (!_useRAF || !_reqAnimFrame) ? function(f) {
                //如果不支持requestAnimationFrame，或你又不打算用它，那就退回用setTimeout
                return window.setTimeout( f, (((_nextTime - _self.time) * 1000 + 1) >> 0) || 1);
            } : _reqAnimFrame;
            _cancelAnimFrame(_id);//每次都要记得清空上次的timeoutID
            _id = _req( _self.tick );
        };
